...
const adminOnly = (req: any, res: any, next: any) => {
  if (req.user?.role !== "admin") {
    return res
      .status(403)
      .json({ error: "Потребан је администраторски приступ." });
  }
  next();
};
...
const nonAdminOnly = (req: any, res: any, next: any) => {
  if (req.user?.role === "admin") {
    return res.status(403).json({
      error: "Администратори имају приступ само административним функцијама.",
    });
  }
  next();
};
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      message: "Корисник је успешно регистрован.",
...
app.post("/api/reset-password", async (req, res) => {
  ...
  if (!username || !token || !newPassword) {
    return res
      .status(400)
      .send("Корисничко име, токен и нова лозинка су обавезни.");
  }
